...
const orcamentoDemo = {
  ...
  observacoes: "Prazo estimado de 2 dias úteis após aprovação. Garantia de 90 dias para peças e mão de obra.",
  itens: [
...
export default function AprovarOrcamentoPage() {
  const [etapa, setEtapa] = useState<"revisar" | "aprovando" | "aprovado" | "reprovando" | "reprovado">("revisar");
  const [motivoRecusa, setMotivoRecusa] = useState("");
  const [showItens, setShowItens] = useState(true);
  const orc = orcamentoDemo;

  function handleAprovar() {
    setEtapa("aprovando");
    setTimeout(() => setEtapa("aprovado"), 1000);
  }
      ...
      <main className="mx-auto max-w-lg space-y-5 px-4 py-8">
        ...
        {etapa === "aprovado" && (
          <div className="rounded-[var(--radius-xl)] border border-[var(--color-success)]/30 bg-[var(--color-success-bg)] p-8 text-center">
            ...
            <p className="mt-4 text-sm font-medium">Prazo estimado: <strong>2 dias úteis</strong></p>
            <a
        ...
        {/* Estado: Revisando / Aprovando / Reprovando */}
        {(etapa === "revisar" || etapa === "aprovando" || etapa === "reprovando") && (
          <>
            ...
            {orc.observacoes && (
              <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-surface)] px-5 py-4 shadow-sm">
                ...
                <p className="text-sm text-[var(--color-fg-muted)]">{orc.observacoes}</p>
              </div>
            )}
              ...
              <div className="rounded-[var(--radius-lg)] border border-[var(--color-warning)]/40 bg-[var(--color-warning-bg)] p-5 space-y-3">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface)] py-2.5 text-sm font-medium hover:bg-[var(--color-surface-muted)] transition-colors"
                  >
                    Cancelar
                  </button>
                  <button
                    onClick={handleConfirmarRecusa}
                    className="flex-1 rounded-[var(--radius-md)] bg-[var(--color-danger)] py-2.5 text-sm font-semibold text-white hover:opacity-90 transition-opacity"
                  >
                    Confirmar recusa
                  </button>
                </div>
            ...
            {etapa !== "reprovando" && (
              <div className="flex flex-col gap-3">
                ...
                <button
                  ...
                  className="w-full rounded-[var(--radius-md)] border border-[var(--color-border)] py-3 text-sm font-medium text-[var(--color-fg-muted)] hover:bg-[var(--color-surface-muted)] transition-colors"
                >
                  Recusar orçamento
                </button>
              </div>
